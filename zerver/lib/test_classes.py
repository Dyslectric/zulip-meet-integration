...
def get_row_pks_in_all_tables() -> Iterator[tuple[str, set[int]]]:
    ...
    # A MigrationsTestCase deliberately leaves the database at an older
    # migration, where tables added by later migrations do not exist yet.
    # Querying those would raise rather than report a leaked row.
    existing_tables = set(connection.introspection.table_names())

    for model in all_models:
        table_name = model._meta.db_table
        if table_name in ignored_tables or table_name not in existing_tables:
            continue
